...
#define Estado_Persiana_topic "EstadoPersianaCocina"
#define Temperatura_topic "TemperaturaCocina"
...
long tiempo_temp=65000;  //tiempo subir o bajar persianas

// Update these with values suitable for your network.
IPAddress ip(192,168,1,195);  //Node static IP
...
void setup() {
  ...
  if (!MDNS.begin("ESP_Persiana_Cocina")) {
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    String clientId = "PersianaCocina";
    // Attempt to connect
    if (client.connect(clientId.c_str())) {
      Serial.println("connected");
      client.subscribe("AccionPersianaCocina"); /*SUSCRIBIRSE A LOS IN TOPICS*/
    } else {
