...
#define Estado_Persiana_topic "EstadoPersianaLucas"
#define Temperatura_topic "TemperaturaLucas"
...
IPAddress ip(192,168,1,205);  //Node static IP
...
void setup() {
  ...
  client.setCallback(callback);
   ArduinoOTA.setHostname("taller");
   
  if (!MDNS.begin("ESP_Persiana_Lucas")) {
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    String clientId = "PersianaLucas";
    // Attempt to connect
    if (client.connect(clientId.c_str())) {
      Serial.println("connected");
      client.subscribe("AccionPersianaLucas"); /*SUSCRIBIRSE A LOS IN TOPICS*/
    } else {
